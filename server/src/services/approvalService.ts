...
export class ApprovalService {
  ...
  private static defaultApprovalChains: PermitApprovalChain[] = [
    {
   ...
   */
  static async getNextApprover(permitId: string, currentStep: number): Promise<any> {
    try {
      ...
      const qualifiedApprovers = potentialApprovers.filter(user => {
        ...
        return nextStep.requiredCertifications.every(requiredCert => 
          userCertNames.includes(requiredCert)
        );
      });
   ...
   */
  static async checkUserCertifications(userId: string, requiredCerts: string[]): Promise<boolean> {
    try {
   ...
   */
  private static async findDelegateApprover(position: string): Promise<any> {
    try {
   ...
   */
  private static getApprovalChain(permitType: string): PermitApprovalChain {
    const chain = this.defaultApprovalChains.find(chain => 
      chain.permitType === permitType
    );
   ...
   */
  static async validateApprovalCompletion(permitId: string): Promise<boolean> {
    try {
      const permit = await prisma.permit.findUnique({
        where: { id: permitId }
      });
   ...
   */
  static async getApprovalProgress(permitId: string): Promise<{
    currentStep: number;
    totalSteps: number;
