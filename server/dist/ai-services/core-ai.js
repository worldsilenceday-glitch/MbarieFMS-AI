...
class MbarieAICore {
     ...
     */
    async recommendDelegation(task) {
        try {
      ...
      `;
            ...
            const recommendedUser = usersWithWorkload.find(user => aiResponse.includes(user.firstName) || aiResponse.includes(user.lastName));
            return {
     ...
     */
    async assessRisk(permitData, incidentHistory) {
        try {
            const prompt = `
        Permit Data:
     ...
     */
    async recommendPermitApproval(permitData) {
        try {
            ...
            const potentialApprovers = users.filter(user => user.department === permitData.department ||
...
exports.mbarieAI = new MbarieAICore();
//# sourceMappingURL=core-ai.js.map